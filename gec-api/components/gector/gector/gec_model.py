...
from components.gector.gector.bert_token_embedder import PretrainedBertEmbedder
from components.gector.gector.seq2labels_model import Seq2Labels
from components.gector.gector.wordpiece_indexer import PretrainedBertIndexer
from components.gector.utils.helpers import PAD, UNK, get_target_sent_by_edits, START_TOKEN
...
class GecBERTModel(object):
    def __init__(self, vocab_path=None, model_paths=None,
        ...
        self.model_weights = list(map(float, weigths)) if weigths else [
            1] * len(model_paths)
        self.gpu_id = gpu_id
    ...
    def predict(self, batches):
        ...
        for batch, model in zip(batches, self.models):
            batch = util.move_to_device(
                ...
                self.gpu_id if torch.cuda.is_available() else -1)
            with torch.no_grad():
    ...
    def _convert(self, data):
        ...
        for output, weight in zip(data, self.model_weights):
            all_class_probs += weight * \
                output['class_probabilities_labels'] / sum(self.model_weights)
            error_probs += weight * \
                output['max_error_probability'] / sum(self.model_weights)
    ...
    def handle_batch(self, full_batch):
        ...
        prev_preds_dict = {i: [final_batch[i]]
