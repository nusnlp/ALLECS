...
from os.path import basename, isdir, isfile, join, splitext, dirname, exists
...
from combinations.esc.file_utils import read_data, _IGNORE_TYPE
...
class M2Dataset(Dataset):
    def __init__(self, m2_dir, data_dir, source_name, target_name, vocab,
                 filter_idx=None, test=False, upsample=None, dict_data=None):
        """
        ...
        if not test and target_name is not None:
            target_path = join(data_dir, target_name)
            target_m2 = read_data(src_path, target_path,
                                  m2_dir, filter_idx=filter_idx)
        else:
            target_m2 = None
        ...
        for file_name in self.hyp_list:
            ...
            if dict_data is not None:
                hyp_name, ext = splitext(file_name)
                hyp_data = dict_data[hyp_name]
            else:
                file_path = join(data_dir, file_name)
                hyp_data = read_data(src_path, file_path,
                                 m2_dir, target_m2, filter_idx)
            data.append(hyp_data)

        doc_lens = [len(d) for d in data]
        assert min(doc_lens) == max(doc_lens), "M2 lengths are different!"

        self.f_size = len(self.edit_types) * len(data)
        ...
        if not test:
            ...
            if upsample is not None:
                try:
                    ...
                    assert ValueError(
                        "Please provide the ratio in the format of class 0:class 1, e.g. 1:2")
                for class_id, ratio in enumerate(ratios):
                    ...
                    if add_ratio > 0:
                        ...
                        print('Found {} instance of class {}, adding {} more'.format(
                            len(label_idx), class_id, num_sample))
    ...
    def transform(self, data, edit_types, test=False):
        ...
        for entity in data:
            hyps = list(entity)
            assert min([hyps[0]['source'] == h['source']
                       for h in hyps]), "Sources are different!"
            ...
            for _, edits in en_edits.items():
                ...
                assert len(feature) == self.f_size
                e_label = -999
...
class Model(nn.Module):
    ...
    def forward(self, x):
        x = self.linear(x)
        x = torch.sigmoid(x)  # nn.functional.sigmoid is deprecated
...
def train(model, train_dataset, batch_size, lr, weight_decay, num_epoch, device,
    ...
    for epoch in range(num_epoch):
        ...
        if eval_dataset is not None:
            ...
            if metric in result:
                score = result[metric]
                if verbose:
                    print('[{}] Accuracy: {}, F0.5: {}'.format(
                        epoch, result['acc'], result['f0.5']))
                if score > best_score:
                    best_score = score
                    ...
                    if model_path is not None:
                        ...
                        print('Model with {} accuracy saved on {}'.format(
                            score, model_path))
            else:
                if verbose:
    ...
    if save_last:
        ...
        torch.save(checkpoint, model_path)
    if verbose:
        print('== best checkpoint ({}) from epoch {} saved in {}'.format(
            best_score, best_epoch, model_path))
...
def eval(model, dataset, device='cpu'):
    ...
    with torch.no_grad():
        ...
        f_half = 0 if precision + \
            recall == 0 else (1 + 0.5 * 0.5) * precision * recall / (0.5 * 0.5 * precision + recall)
...
def test(model, model_path, dataset, device, threshold=0.5,
    ...
    all_outputs = [None if l[0] is None else torch.stack(
        list(l), dim=0).mean(dim=0) for l in all_outputs]
    ...
    for idx, output in enumerate(all_outputs):
        ...
        edits_to_apply = sorted(
            edits_to_apply, key=lambda x: x[3], reverse=True)
        filtered_edits = []
        def multiple_insertion(x, y): return x[0] == x[1] == y[0] == y[1]
        def intersecting_range(x, y): return (x[0] <= y[0] < x[1] and not x[0] == y[1]) or \
        ...
        if generate_text:
            ...
            for edit in filtered_edits:
                ...
                for h_idx, e_type, _label in values:
                    model_name = dataset.hyp_list[h_idx]
                    edit_info['bases'].append(model_name)
                    if e_type not in _IGNORE_TYPE and \
                        not e_type.endswith('OTHER') and \
                            edit_info['type'] is None:
                        edit_info['type'] = e_type
                final_edits.append(edit_info)
            result[idx] = {
                    'source': source,
                    'edits': final_edits,
                }
...
def main(args):
    ...
    if args.train:
        ...
        kf = KFold(n_splits=args.val_ratio,
                   shuffle=True, random_state=args.seed)
        dummy_file = [1 for _ in open(
            join(args.data_dir, args.source_name), encoding='utf-8')]
...
def predict(data, generate_text=True):
    exp_dir = 'bea-exp'  # using BEA-2019
    ...
    device = torch.device(device_str)
    vocab_path = join(dirname(__file__), exp_dir, 'vocab.idx')
    with open(vocab_path, 'r', encoding='utf-8') as f:
        vocab = json.load(f)
    source_name = 'source.txt'
    target_name = 'target.txt'
    test_dataset = M2Dataset(None,
                             None,
                             source_name,
                             target_name,
                             vocab,
    ...
    feature_size = test_dataset.feature_size()
    model = Model(feature_size).to(device)
    model_path = join(dirname(__file__), exp_dir, 'models', 'model.pt')
    sentences = test(model, model_path, test_dataset, device,
                    generate_text=generate_text)  # use default threshold=0.5
    return sentences
...
def get_arguments():
    ...
    parser.add_argument('--m2_dir', default='m2',
                        help='path to the generated m2 files')
    parser.add_argument('--source_name', default='source.txt',
                        help='The source filename')
    parser.add_argument('--target_name', default='target.txt',
                        help='The target filename')
    parser.add_argument('--lr', type=float, default=0.1, help="learning rate")
    parser.add_argument('--weight_decay', type=float,
                        default=0, help="weight decay (L2 penalty)")
    parser.add_argument('--seed', type=int, default=0, help="random seed")
    parser.add_argument('--val_ratio', type=int, default=5,
                        help="1/val_ratio of the data is for validation")
    parser.add_argument('--threshold', type=float,
                        default=0.5, help="probability threshold")
    parser.add_argument('--upsample', type=str, default=None,
                        help='up-sample ratio of class 0:class 1')
    parser.add_argument('--train', default=False,
                        action='store_true', help='train the model')
    parser.add_argument('--test', default=False,
                        action='store_true', help='test the model')
    parser.add_argument('--target_path', 
                        help='path to the target file during training')
    parser.add_argument('--vocab_path', default='vocab.idx',
